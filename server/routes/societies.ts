...
export const getAllSocieties: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
    ...
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can view all societies' });
    }
...
// Get society by ID
export const getSocietyById: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
...
export const createSociety: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can create societies' });
    }
    ...
    if (isMongoConnected) {
      ...
      if (existingSociety) {
        return res.status(409).json({ error: 'Society with this name already exists' });
      }
...
export const updateSociety: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can update societies' });
    }
...
export const createSocietyUser: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can create society users' });
    }
    ...
    if (!name || !email || !role || !societyId) {
      return res.status(400).json({ error: 'Name, email, role, and society ID are required' });
    }
    ...
    const userData = {
      ...
      role,
      societyId,
      permissions: permissions || { canRead: true, canWrite: role === 'society_user' },
...
export const getSocietyUsers: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    if (isMongoConnected) {
      ...
      users = MockDataService['users'].filter(user => user.societyId === societyId);
      // Remove passwords
...
export const updateUserPermissions: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    if (!permissions || typeof permissions.canRead !== 'boolean' || typeof permissions.canWrite !== 'boolean') {
    ...
    if (isMongoConnected) {
      user = await User.findByIdAndUpdate(
        ...
        { new: true }
      ).select('-password').populate('societyId', 'name');
    } else {
