...
export const getTransactionById: RequestHandler = async (
  ...
  try {
    ...
    // Check permissions
    if (
      req.user.role === "society_user" &&
      transaction.societyId?.toString() !== req.user.societyId?.toString()
    ) {
    ...
    if (
      req.user.role === "agent" &&
      transaction.assignedToAgent?.toString() !== req.user._id.toString()
    ) {
      return res.status(403).json({ error: "Access denied" });
    }
...
export const createTransaction: RequestHandler = async (
  ...
  try {
    ...
    if (req.user.role !== "society_user") {
      return res
        .status(403)
        .json({ error: "Only society users can create transactions" });
    }
    ...
    if (!vendorName || !nature) {
      return res
        .status(400)
        .json({ error: "Vendor name and nature are required" });
    }
  ...
  try {
    ...
    if (req.user.role === "society_user") {
      ...
      if (transaction.status !== "pending_for_clarification") {
        return res
          .status(403)
          .json({ error: "Cannot update transaction in current status" });
      }
    } else if (req.user.role === "agent") {
    ...
    if (remark) {
      const newRemark = {
        ...
        type:
          status === "completed"
            ? ("approval" as const)
            : status === "pending_for_clarification"
...
export const assignTransactionToAgent: RequestHandler = async (
  req: AuthenticatedRequest,
  res,
) => {
  try {
    ...
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only admins can assign transactions" });
    }
