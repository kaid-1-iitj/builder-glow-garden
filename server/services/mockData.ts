...
export class MockDataService {
  ...
  static async getTransactions(filters?: {
    ...
    if (filters) {
      if (filters.societyId) {
        transactions = transactions.filter(t => t.societyId === filters.societyId);
      }
      if (filters.assignedToAgent) {
        transactions = transactions.filter(t => t.assignedToAgent === filters.assignedToAgent);
      }
      if (filters.status) {
        transactions = transactions.filter(t => t.status === filters.status);
      }
      if (filters.createdBy) {
        transactions = transactions.filter(t => t.createdBy === filters.createdBy);
      }
    }
  ...
  static async updateTransaction(id: string, updates: Partial<ITransaction>) {
    await this.initialize();
    const transactionIndex = this.transactions.findIndex(transaction => transaction._id === id);
  ...
  static async addRemarkToTransaction(transactionId: string, remark: {
    text: string;
    author: string;
  ...
  // Statistics
  static async getDashboardStats(userRole: string, userId: string, societyId?: string) {
    await this.initialize();
    ...
    if (userRole === 'admin') {
      return {
        ...
        pendingTransactions: this.transactions.filter(t => t.status !== 'completed').length
      };
    }
    
    if (userRole === 'society_user' && societyId) {
      const societyTransactions = this.transactions.filter(t => t.societyId === societyId);
      return {
        myTransactions: societyTransactions.filter(t => t.createdBy === userId).length,
        pendingTransactions: societyTransactions.filter(t => t.status !== 'completed').length,
        completedTransactions: societyTransactions.filter(t => t.status === 'completed').length,
        pendingClarifications: societyTransactions.filter(t => t.status === 'pending_for_clarification').length
      };
    }
    
    if (userRole === 'agent') {
      const agentTransactions = this.transactions.filter(t => t.assignedToAgent === userId);
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      
      return {
        assignedTransactions: agentTransactions.length,
        pendingReview: agentTransactions.filter(t => t.status === 'pending_on_agent').length,
        completedToday: agentTransactions.filter(t => 
          t.status === 'completed' && 
          t.completedAt && 
